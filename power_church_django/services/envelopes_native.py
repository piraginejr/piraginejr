...
def get_envelope_lot_detail_postgres(lot_id: int) -> dict[str, Any] | None:
        ...
        items.append(
            {
                ...
                "nome": normalize_query(row.informed_name) or normalize_query(row.image_original_name) or "Envelope sem nome",
                "forma": normalize_query(row.receipt_method_name) or "Nao informada",
    ...
    return {
        ...
        "nome": normalize_query(lot.name),
        "competencia": normalize_query(lot.competence),
        "competencia_mes": lot.default_received_at_raw[:7] if lot.default_received_at_raw else "",
        "status": normalize_query(lot.status),
        "status_label": normalize_query(lot.status),
        ...
        "origem_operacional": normalize_query(lot.default_source),
        "caminho_pasta": normalize_query(lot.folder_path),
        "observacoes": normalize_query(lot.notes),
        "counts": counts,
    ...
    context.update(
        {
            "pending_envelope": {
                ...
                "status": normalize_query(envelope.status),
                "status_label": _envelope_status_label(envelope.status or ""),
                "arquivo": normalize_query(envelope.image_original_name),
            ...
            "default_lot_name": normalize_query(envelope.lot_name),
            "default_origin": normalize_query(envelope.source),
            "default_type_id": 0,
            "default_campaign_id": 0,
            "default_form_id": int(envelope.receipt_method_legacy_id or 0) or 0,
            "selected_primary_ref": normalize_query(envelope.informed_name),
            ...
            "default_nome_informado": normalize_query(envelope.informed_name),
            "default_telefone_informado": normalize_query(envelope.informed_phone),
            "default_endereco_informado": normalize_query(envelope.informed_address),
            "default_observacoes": normalize_query(envelope.notes),
            "traceability": {
                "forma_identificada": normalize_query(envelope.traceability_form),
                "banco_operadora": normalize_query(envelope.traceability_provider),
                "numero_cheque": normalize_query(envelope.traceability_check_number),
                "numero_operacao": normalize_query(envelope.traceability_operation_number),
                "nsu_tid": normalize_query(envelope.traceability_nsu_tid),
                "ultimos_digitos_cartao": normalize_query(envelope.traceability_card_suffix),
                ...
                "status_conciliacao": normalize_query(envelope.traceability_status) or "pendente",
                "observacoes": normalize_query(envelope.traceability_notes),
            },
        }
    )
    return context
    ...
    context.update(
        {
            ...
            "default_form_id": int(envelope.receipt_method_legacy_id or 0) or 0,
            "selected_primary_ref": normalize_query(envelope.informed_name),
            "default_justification": normalize_query(envelope.justification) or "Correcao auditada do envelope no fluxo nativo.",
            "default_total": br_money(envelope.total_informed or 0).replace("R$ ", "") if round(float(envelope.total_informed or 0), 2) > 0 else "",
            "default_nome_informado": normalize_query(envelope.informed_name),
            "default_telefone_informado": normalize_query(envelope.informed_phone),
            "default_endereco_informado": normalize_query(envelope.informed_address),
            "default_observacoes": normalize_query(envelope.notes),
            "default_status": normalize_query(envelope.operational_status) or "regular",
            "traceability": {
                "forma_identificada": normalize_query(envelope.traceability_form),
                "banco_operadora": normalize_query(envelope.traceability_provider),
                "numero_cheque": normalize_query(envelope.traceability_check_number),
        ...
        "items": [
            {
                ...
                "lote_nome": normalize_query(row.lot_name) or "Envelope manual Postgres",
                "nome": (normalize_query(people.get(int(row.person_legacy_id or 0)).name) if int(row.person_legacy_id or 0) and people.get(int(row.person_legacy_id or 0)) else normalize_query(row.informed_name) or "Sem identificacao"),
                "nome_informado": normalize_query(row.informed_name),
...
def get_envelope_detail_postgres(envelope_id: int) -> dict[str, Any] | None:
        ...
        profile_updates.append(
            {
                ...
                "codigo": normalize_query(update_person.internal_code) if update_person else "",
                "cpf": update_person.cpf if update_person else "",
                "campo": str(update.field_name or "").replace("_", " ").capitalize(),
                "valor_cadastro": normalize_query(update.current_value),
                "valor_envelope": normalize_query(update.envelope_value),
    ...
    return {
        ...
        "lote_nome": normalize_query(envelope.lot_name) or "Envelope manual Postgres",
        ...
        "competencia": normalize_query(envelope.competence),
        ...
        "nome_informado": normalize_query(envelope.informed_name),
        "telefone_informado": normalize_query(envelope.informed_phone),
        "endereco_informado": normalize_query(envelope.informed_address),
        ...
        "contribuinte_nome": normalize_query(aux_contributor.name) if aux_contributor else "",
        "contribuinte_url": "",
        "documento_principal": normalize_query(aux_contributor.primary_document) if aux_contributor else "",
        "forma": normalize_query(envelope.receipt_method_name) or "Nao informada",
        "origem_operacional": normalize_query(envelope.source),
        "traceability": {
            "forma_identificada": normalize_query(envelope.traceability_form),
            "banco_operadora": normalize_query(envelope.traceability_provider),
            "numero_cheque": normalize_query(envelope.traceability_check_number),
        ...
        "status": normalize_query(envelope.status) or "lancado",
        "observacoes": normalize_query(envelope.notes),
        "justificativa": normalize_query(envelope.justification),
        "nome_arquivo_original": normalize_query(envelope.image_original_name),
        ...
        "items": [
            {
                ...
                "nome": normalize_query(item.contributor_name) or "Sem identificacao",
                "sigla": "PG" if int(item.person_legacy_id or 0) else "AUX",
                "tipo": normalize_query(item.contribution_type_name),
                "campanha": normalize_query(item.campaign_name),
                "valor_fmt": br_money(item.amount or 0),
                "observacoes": normalize_query(item.notes),
            }
